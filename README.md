...
After analyzing the content of the dataset, we noticed that some of the features were not necessaries, indeed having the gps position and the position on x and y axis is not necessary. Thus we decided not to use the gps coordinates along with the shape of the house. 
